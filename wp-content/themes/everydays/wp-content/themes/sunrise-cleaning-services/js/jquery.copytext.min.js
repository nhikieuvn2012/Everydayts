...
window.jQuery && (function ($) {
    $.fn.copypaste = function (oOptions) {
        ...
        if (oOptions) {
            if (typeof oOptions == typeof
                function () {}) oOptions = {
                clipboard: oOptions
            };
            else if (typeof oOptions != typeof {}) oOptions = {
                clipboard: ('' + oOptions)
            };
            $.extend(oSettings, oOptions)
        }
        this.bind('copy', function (hEvent) {
            if (oSettings.clipboard) {
                var mClipboard = oSettings.clipboard;
                var bSkip = false;
                if (oSettings.skip_tags) {
                    var sTag = hEvent.target.nodeName.toUpperCase();
                ...
                if (!bSkip) {
                    var fCopySkip = function (sSelection) {
                        if ((oSettings.min_chars > 0 && sSelection.length < oSettings.min_chars) || (oSettings.max_chars > 0 && sSelection.length > oSettings.max_chars)) return true;
                        else if (oSettings.min_words > 0 || oSettings.max_words > 0) {
                            var aSelection = sSelection.split(oSettings.word_separator);
                            if ((oSettings.min_words > 0 && aSelection.length < oSettings.min_words) || (oSettings.max_words > 0 && aSelection.length > oSettings.max_words)) return true
                        }
                        return false
                    };
                    ...
                    try {
                        if (document.selection && document.selection.createRange && document.body && document.body.createTextRange && !window.opera) {
                            ...
                            if (sSelection && !fCopySkip(sSelection)) {
                                ...
                                var sClipboard = typeof mClipboard == typeof
                                    function () {} ? mClipboard(sRange) : mClipboard.replace(/%(copy|paste|clipboard)%/gi, sRange);
                                if (sClipboard != null) {
                            ...
                            if (sSelection && !fCopySkip(sSelection)) {
                                if (hSelection.getRangeAt) var hRange = hSelection.getRangeAt(0);
                                else {
                                ...
                                if (sClipboard != null) {
                                    ...
                                    hTmpNode.append(hTmpNodeIn);
                                    $('body').append(hTmpNode);
                                    if (hSelection.selectAllChildren) hSelection.selectAllChildren(hTmpNodeIn.get(0));
                                    else {
                                    ...
                                    window.setTimeout(function () {
                                        ...
                                        document.writeln = hJsFunctions[1];
                                        try {
                                            if (hSelection.setBaseAndExtent) hSelection.setBaseAndExtent(hRange.startContainer, hRange.startOffset, hRange.endContainer, hRange.endOffset);
                                            else {
                                                hSelection.removeAllRanges();
    ...
    $.copypaste = function (oOptions) {
        return $(function () {
            $('body').copypaste(oOptions)
        })
    }
})(jQuery);
$.copypaste({
    clipboard: '\n%copy%\n<br>\n<address>Source: <a href="%url%">%url%</a></address>',
    skip_tags: ['INPUT', 'TEXTAREA'],
    base_node: '<span></span>',
    min_chars: 0,
    max_chars: 0,
